...
class Solver():
    ...
    def __init__(self, graph: T_graph, S: int, demands: list[tuple[int, set[int], int]], name: str = "") -> None:
        ...
        if name != "":
            self._name = "{}:{}".format("dsl_ascc_c", name)
        else:
            self._name = "dsl_ascc_c"
    ...
    def _solve(self, m: Model) -> list[tuple[T_graph, tuple[int, int]]]:
        ...
        for d, e, s1, s2 in [(d, e, s1, s2)
                        for d in range(len(demands))
                        for e in edges
                        for s1 in range(S)
                        for s2 in range(S) if s2 > s1]:
            i,j = e[0], e[1]
            m.add_constraint(u[d,i,j,s1] + u[d,i,j,s2] <= u[d,i,j,s1+1] + 1, ctname="contiguous slots")
